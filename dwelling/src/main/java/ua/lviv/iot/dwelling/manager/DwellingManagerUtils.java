...
public class DwellingManagerUtils {
	...
	public static void sortByArea(List<AbstractDwelling> dwellings, SortType sortType) {
		DwellingManagerUtils utils = new DwellingManagerUtils();
		dwellings.sort(sortType == SortType.ASC ? utils.new DwellingSorterByArea()
				: utils.new DwellingSorterByArea().reversed());
	}
	...
	public static void sortByDistrict(List<AbstractDwelling> dwellings, SortType sortType) {
		switch (sortType) {
		case ASC:
			dwellings.sort((AbstractDwelling firstDwelling,
					AbstractDwelling secondDwelling) -> firstDwelling.getDistrict().compareTo(secondDwelling.getDistrict()));
			break;
		case DESC:
			dwellings.sort((AbstractDwelling firstDwelling,
					AbstractDwelling secondDwelling) -> secondDwelling.getDistrict().compareTo(firstDwelling.getDistrict()));
			break;
		default:
			break;
		}
